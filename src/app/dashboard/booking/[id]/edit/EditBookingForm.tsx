...
export default function EditBookingForm({ booking }: { booking: Booking }) {
  ...
  const startDateObj = new Date(booking.startAt);
  const endDateObj = new Date(booking.endAt);
  ...
  const [startAt, setStartAt] = useState<string>(booking.startAt || "");
  const [endAt, setEndAt] = useState<string>(booking.endAt || "");
  ...
  const [startTime, setStartTime] = useState<string>(() => {
    const h = String(startDateObj.getHours()).padStart(2, "0");
  ...
  const [endTime, setEndTime] = useState<string>(() => {
    const h = String(endDateObj.getHours()).padStart(2, "0");
    const m = String(endDateObj.getMinutes()).padStart(2, "0");
    return `${h}:${m}`;
  });
  ...
  const [bookingDiscount, setBookingDiscount] = useState<number>(booking.discountAmount || 0);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        items: items
          ...
          .map((x) => ({
            ...
            units: x.priceType === "per_unit" || x.priceType === "per_hour" ? x.units : undefined,
      ...
      if (res.ok) {
        alert(t("bookingUpdated"));
        window.location.href = `/dashboard/booking/${booking._id}`;
      } else {
